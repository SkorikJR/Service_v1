    ...
    public partial class All_Rem : Form
    {
        ...
        private void All_Rem_Load(object sender, EventArgs e)//Нужно довести до ума!
        {
            ...
            for (int i = 0; i < Auth.ВсегоРемонтов; i++)
            {
                ...
                for (int ячейка = 0; ячейка < Столбцы; ячейка ++)
                {
                    ...
                    if (ячейка == 4)
                    {
                        Auth.Remont_All[i, ячейка] = DateTime.Parse(Auth.Remont_All[i, ячейка]).ToString("dd'.'MM'.'yyyy");
                    }
                    if (ячейка == 5)
                    {
        ...
        private void DataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (Столбец == "0")
            {
                ...
                {
                    //if (ячейка == 6| ячейка == 27| ячейка == 14| ячейка == 19)
                    //{
                        if (ДанныеДляОтбора.Ремонт[ячейка] == "0000-00-00")
                        {
                            ДанныеДляОтбора.Ремонт[ячейка] = "2000-01-01";
                        }
                        //ДанныеДляОтбора.Ремонт[ячейка] = DateTime.Parse(Результат[ячейка].ToString()).ToString("dd'.'MM'.'yyyy");
                    //}
                    //else
                    //{
